...
func buildScriptPurpose(
	...
	case 2: // Reward
		...
		return data.NewConstr(2, data.NewConstr(0))
	case 3: // Cert
		// TODO: implement certifying purpose
		return data.NewConstr(3, data.NewConstr(0))
	}
	// Default fallback
	return data.NewConstr(0)
}
...
func buildTxInfo(
	...
	for key, value := range redeemers.Iter() {
		purpose := buildScriptPurpose(key.Tag, key.Index, tx)
...
func buildScriptContext(
	...
	purpose := buildScriptPurpose(redeemerTag, redeemerIndex, tx)
	return data.NewConstr(0, txInfo, purpose)
}
...
func (s *submitServiceServer) EvalTx(
	...
	// Resolve UTxOs for inputs to get datums
	resolvedUtxos := make(map[string]ledger.Utxo)
	for _, input := range tx.Inputs() {
		utxos, err := oConn.LocalStateQuery().Client.GetUTxOByTxIn(
			[]ledger.TransactionInput{input},
		)
		if err != nil {
			return connect.NewResponse(
					resp,
				), fmt.Errorf(
					"failed to query UTxO for input %s: %w",
					input.String(),
					err,
				)
		}
		for k, v := range utxos.Results {
			_ = k // k is UtxoId
			resolvedUtxos[input.String()] = ledger.Utxo{Id: input, Output: v}
		}
	}

	// Resolve UTxOs for reference inputs (needed for script context)
	for _, input := range tx.ReferenceInputs() {
		utxos, err := oConn.LocalStateQuery().Client.GetUTxOByTxIn(
			[]ledger.TransactionInput{input},
		)
		if err != nil {
			return connect.NewResponse(
					...
					"failed to query UTxO for reference input %s: %w",
					input.String(),
					err,
				)
		}
	...
	for _, pair := range pairs {
		...
		if uint8(key.Tag) == 0 { // RedeemerTagSpend
			...
			contextData := buildScriptContext(
				tx,
				resolvedUtxos,
				redeemers,
				key.Tag,
